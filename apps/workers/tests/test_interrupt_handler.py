...
- _wait_via_redis: pub/sub message receipt, ignore non-message events
...
class TestWaitViaPolling:
    ...
    async def test_raises_timeout_when_unresolved(self) -> None:
        """Polling exits with TimeoutError when elapsed > timeout."""
        handler = _make_handler()
        pending = _resp(200, {"id": "req-1", "status": "pending"})
        handler.client = MagicMock()
        handler.client.get = AsyncMock(return_value=pending)

        # Use no-op sleep so test runs fast; elapsed accumulates each iteration.
...
class TestWaitViaRedis:
    ...
    async def test_ignores_non_message_pubsub_events(self) -> None:
        """get_message returning None should be skipped, then the real message returned."""
        handler = _make_handler()
        msg_payload = {
            "request_id": "req-1",
            "result": "denied",
    ...
    @pytest.mark.asyncio
    async def test_unsubscribes_in_finally_block_on_exception(self) -> None:
        """If subscribe raises, the finally block still runs unsubscribe + close."""
        handler = _make_handler()
        ...
        with patch(
            ...
            with pytest.raises(RuntimeError, match="subscribe failed"):
                await handler._wait_via_redis("req-1", timeout=5)

        # finally block must execute even on exception
        pubsub.unsubscribe.assert_awaited_once()
        pubsub.close.assert_awaited_once()
...
class TestWaitForApproval:
    ...
    async def test_redis_success_short_circuits(self) -> None:
        """Successful redis path skips polling."""
        handler = _make_handler()
        polling_mock = AsyncMock()
        handler._wait_via_redis = AsyncMock(  # type: ignore[method-assign]
            return_value=ApprovalResponse(request_id="r", result="approved")
        )
        handler._wait_via_polling = polling_mock  # type: ignore[method-assign]
