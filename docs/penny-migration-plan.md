...
**Status:** In execution — Phases A + B landed (see §8); C–F pending; G awaits owner (RFC 0024 P4, owner-approved)
...
Subtotal: **~946 lines**. These are the _producers_ of artifacts. selva already
...
## 8. Execution log (2026-07-06 — Phases A + B)

**Landed** (branch `feat/artifacts-phase-a-b`):

- **Phase A — types.** Penny's `packages/types/src/artifacts/index.ts`
  ported into `packages/shared-types/src/artifact.ts` ("fold into
  shared-types" option). Deliberate adaptations, since selva-office has
  no zod dependency and shared-types is hand-written interfaces:
  - Zod schemas → plain TS interfaces + narrow runtime guards
    (`isArtifact`, `isArtifactType`) and content coercion helpers
    (`markdownSource`, `htmlSource`) — the only runtime validation
    Phase B consumes. Adding zod for one module was rejected as
    non-idiomatic.
  - `z.date()` → ISO-8601 strings (shared-types convention).
  - Penny's tenant envelope (`tenantId`, `conversationId`, `createdBy`,
    `isPublic`) collapsed to optional `createdBy`/`sourceId` provenance —
    selva's org/agent model owns identity.
  - `metadata.ts` / `renderers.ts` / `versions.ts` (~812 lines) **not**
    ported yet: their consumers are the Phase C/D/E surfaces
    (renderer-config plumbing, version history). Porting them now would
    land dead types; re-visit per phase.
- **Phase B — first renderers, flagged.** Both §5.2 candidates ported
  into `apps/office-ui/src/components/artifacts/`
  (`ArtifactViewer.tsx` dispatcher + error boundary,
  `MarkdownArtifactRenderer.tsx`, `HtmlArtifactRenderer.tsx`), with a
  flag-gated QA mount at `apps/office-ui/src/app/artifacts/page.tsx`
  and `NEXT_PUBLIC_ARTIFACTS_ENABLED=false` in `.env.example`. Tests in
  `apps/office-ui/src/components/__tests__/ArtifactViewer.test.tsx` and
  `packages/shared-types/src/__tests__/artifact.test.ts`.
  Security hardening applied on port (do not regress in Phase C/D):
  - Penny's HTMLRenderer `document.write` + `sandbox="allow-scripts
allow-same-origin"` (which voids the sandbox) → `srcDoc` +
    `allow-scripts` at most, never `allow-same-origin`.
  - Penny's MarkdownRenderer piped unescaped input into
    `dangerouslySetInnerHTML` → source is HTML-escaped before the
    markdown transforms; link hrefs are scheme-allowlisted
    (`https?:`/`mailto:`/`#`/`/`); heading ids slugified so TOC anchors
    resolve (penny bug).
  - ArtifactHeader/export/share chrome intentionally not ported
    (Phase D, per §5.4).

**Still pending:** Phases C (remaining renderers + chart-lib decision),
D (interactive shell/store), E (persistence — only if required),
F (producers), and G (archive — **owner action**: penny CI is now a
no-op and its README is terminal; repo archiving via GitHub settings is
the final owner step).
